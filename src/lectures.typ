...
$ P_k^0 (mu) = product_(j=1)^n (1 - tau_j^0 mu), space mu_j = 1/tau^0_j $

$ T_k ((2mu - (alpha+beta))/(beta-alpha) / t_n $

$ mu_j = (alpha + beta) / 2 + (beta - alpha)/2 cos ((2j - 1) pi)/(2k) $

$ tau_j^0 = 2 / ((alpha+beta + (beta - alpha) cos ((2j-1)pi)/(2k))), space j = 1, dots, k $

$ norm(T_k)_2 <= 1/abs(t_k) $

Найдём количество шагов, чтобы начальную погрешность уменьшить в $e$ раз.

$ norm(z^((0)))_2  $

$ 1 / abs(t_k) <= 1/e $

$ norm(z^((k)))_2 <= norm(T_k)_2 norm(z^((0)))_2 $

$ norm(z^((k)))_2 <= 1/abs(t_k) norm(z^((0)))_2 <= 1/e norm(z^((0)))_2 $

$ abs(t_k) >= e $

$ t_k = T_k (- (alpha + beta)/(beta- alpha)) $

$ -(alpha + beta)/(beta-alpha)$ выпадает за отрезок $[-1, 1]$. Поэтому используем другое представление полинома Чебышёва:

$ T_k (x) = ((x-sqrt(x^2 - 1))^k + (x +sqrt(x^2 - 1))^k)/2 $

Подставляем сюда $x=-(alpha+beta)/(beta-alpha)$.

$ x - sqrt(x^2-1) = - (sqrt(beta) + sqrt(alpha))/(sqrt(beta)-sqrt(alpha)) $

$ x + sqrt(x^2-1) = - (sqrt(beta) - sqrt(alpha))/(sqrt(beta)+sqrt(alpha)) $

$ rho := (sqrt(beta) + sqrt(alpha))/(sqrt(beta)-sqrt(alpha)) > 1 $

$ t_k = T_k(x) = 1/2 [ (-1)^k rho^k + (-1)^k/rho^k] = (-1)^k /2 (rho^(2k) + 1)/rho^k $

Смотрим по модулю:

$ (rho^2k + 1)/(2 rho^k) >= e $

Теперь порешаем это неравенство.

$ rho^(2k) + 1 >= 2rho^k e $

$ rho^(2k) - 2 rho^k e + e^2 >= e^2 - 1 $

$ (rho^k - e)^2 >= e^2 - 1  $

$ rho^k >= e + sqrt(e^2-1) $

$ k ln rho >= ln(e + sqrt(e^2 - 1)) $

$ k >= ln(e+sqrt(e^2-1))/(ln (sqrt(beta)+sqrt(alpha))/(sqrt(beta)-sqrt(alpha)) ) $

$ k >= 1/ (ln (beta+alpha)/(beta-alpha) ) $

Рассмотрим пример, когда у нас не очень хорошая матрица:

$ alpha = 10^(-3) quad beta = 1 $

$ mu_2 (A) = 1 / (10^(-3)) = 10^3 $

Если мы сейчас посчитаем число шагов в методе Ричардсона и методе простой итерации, то:
- Простая итерация --- порядка 500 шагов.
- Метод итерация --- порядок 26 шагов.


Метод применяют в так называемом циклическом виде. А именно. Фиксируют $k$.
Например $k=10$. Считаем $t_j^0$. Пока не получим решение с нужной нам точностью $epsilon$.

Но это ещё не всё. Обычно такие сложные нестационарные методы в себе содержат вычислительную неустойчивость. Всё сделаем, начинаем считать и накапливается вычислительная ошибка. Обычно в процессе вычисления ошибка съедается, но здесь ситуация друга. Почему этот метод может быть неустойчивым?

Вот возьмём один шажок:

$ z^((j)) = (I - tau_j^0 A) z^((j-1)), space j=1, ..., k $

Но это если мы не учитываем погрешности машинной арифметики. Реально происходит:

$ tilde(z)^((j)) = (I - tau_j^0 A) tilde(z)^((j-1)) + rho^j $

$ z^((k)) = (I - tau_k^0) z^((k-1)) + rho^((k)) =\
  = (I - tau_k^0 A)(I-tau_(k-1)^0 A)z^((k-2)) + (I-tau_k^0 A)rho^((k-1)) + rho^k =\
  =T_k z^((0)) + sum_(i=1)^k (product_(j=i+1)^n (I-tau_j^0 A)) rho^((i))
$

$ norm(T_k)_2 <= 1/abs(t_k) = (2rho^k)/(1+rho^(2k)) < 1 $


== Метод наибыстрейшего градиентного спуска.

Идея метода. Есть некая функция $F(x)$. Где $x$ --- это вектор. Общая формула:

$ x^((k+1)) = x^((k)) - delta_k gradient F(x^((k))) $

Градиент смотрит в сторону возрастания, поэтому мы берём со знаком минус.

$delta_k$ находится из условия минимума:

$ F(x^((k+1))) = F(x^((k)) - delta_k gradient F(x^((k)))) $

Как это связано с решением СЛАУ?

$ A y = f $

Введём функцию многих переменных:

$ F(y) = (A y, y) - 2 (f, y) $

Ограничения:

$ A = A^* > 0, space A - "вещественная" $
...
$ gradient F = ( (diff F)/(diff x_1), (diff F)/(diff x_2), dots, (diff F)/(diff x_n) )  $

$ F = sum_(i=1)^n (A x_i) x_i - 2 sum_(i=1)^n f_i x_i $

$ sum_(j=1)^n sum_(i=1)^n a_(i j) x_j x_i - 2 sum_(i=1)^n f_i x_i $

$ (diff F)/(diff x_i) = 2(A x_i - f_i) $

$ gradient F = 2 (A x - f) $

$ x^((k+1)) = x^((k)) - 2 delta_k (A x^((k)) - f) $

$ 2 delta_k := Delta_k $

$ x^((k-1)) = x^((k)) - Delta_k (A x^((k)) - f) := phi(Delta_k) $

$ phi'(Delta_k) = 0 $

$ phi'(Delta_k) = [(A x ^((k+1)), x^((k+1)))-2 (f, x^((k+1)))]' = \
  = (A (dif x^((k+1)))/(dif Delta_k), x^((k+1))) + (A x^((k+1)), (dif x^((k+1)))/(dif Delta_k)) - 2 (f, (dif x^((k+1)))/(dif Delta_k)) = \

  = 2 (A x^((k+1)) - f, (dif x^((k+1)))/(dif Delta_k)) = 0
$

$ (dif x^((k+1)))/(dif Delta_k) = - (A x^((k)) - f) $

$ - 2 (A x^((k+1)) - f, A x^((k)) - f) = 0 $

$ ( A x^((k)) - f - Delta_k A ( A x^((k)) - f), A x^((k)) - f ) = 0 $

$ Delta_k = ((A x^((k)), A x^((k)) -f))/((A (A x^((k)) - f), A x^((k)) -f)) $

На самом деле метод похож на метод простой итерации.

Вопросы, связанные со сходимостью. Теорема такая:

$ F_0 (y) := F(y) + (A x^*, x^*) = (A(y-x^*), y-x^*) $
...
$ F_0 (x^((k))) <= ((beta - alpha)/(beta+alpha))^(2n) F_0 (x^((0))) $

$ alpha = min_(i) lambda_i (A) wide beta = max_i lambda_i (A) $

\

$ norm(z^((k)))_2 <= ((beta-alpha)/(beta+alpha))^n sqrt(beta/alpha) dot norm(z^((0)))_2 $

$ z^((k)) = sum_(i=1)^n c_i u^((i)) $

$ norm(z_k)^2_2 = sum_(i=1)^n c_i^2 $

$ (A z^((k)), z^((k))) = sum_(i=1)^n lambda_i c_i^2 <= beta sum_(i=1)^n c_i^2 = beta norm(z^((k)))_2 $

$ alpha  norm(z^((k)))_2^2 <= F_0 (x^((k))) <= beta norm(z^((k)))_2^2 $

$ norm(z^((k)))_2 <= sqrt((F_0 (x^(k)))/alpha) <= sqrt(1/alpha  ((beta-alpha)/(beta+alpha))^(2 n) F_0 (x^((0)))) <= $

На этом вторая часть окончена. Можно сдавать коллоквиум по второй части. Далее будет третья часть, это проблема собственных значений.